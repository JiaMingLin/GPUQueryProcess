...
__global__
void parallelAggregate( Record* d_S, int* d_startPos, int* d_aggResults, int OPERATOR, int blockOffset, int numGroups, int rLen  )
{
	int tx = threadIdx.x;
...
void gatherBeforeAgg( Record* d_Rin, int rLen, Record* d_Ragg, Record* d_S, int numThread, int numBlock )
{
	...
	int* h_loc = (int*)malloc( sizeof(int)*rLen );
	Record* h_S = (Record*)malloc( sizeof(Record)*rLen );
	CUDA_SAFE_CALL( cudaMemcpy( h_Rin, d_Rin, sizeof(Record)*rLen, cudaMemcpyDeviceToHost ) );
	CUDA_SAFE_CALL( cudaMemcpy( h_loc, d_loc, sizeof(int)*rLen, cudaMemcpyDeviceToHost ) );
	CUDA_SAFE_CALL( cudaMemcpy( h_S, d_S, sizeof(Record)*rLen, cudaMemcpyDeviceToHost ) );*/
}
						 ...
						 int numThread = 256)
{
	...
	//CUDA_SAFE_CALL( cudaMemcpy( h_aggResults, d_aggResults, sizeof(int)*numGroups, cudaMemcpyDeviceToHost ) );

...
unsigned int g_startTime=0;
void startTime()
{
	CUT_SAFE_CALL( cutCreateTimer( &g_startTime));
    CUT_SAFE_CALL( cutStartTimer( g_startTime));
}
double endTime(char *info)
{
	cudaThreadSynchronize();
	CUT_SAFE_CALL( cutStopTimer( g_startTime));
	double result=cutGetTimerValue(g_startTime);
	printf("***%s, time, %f, ms***\n", info, result);
    CUT_SAFE_CALL( cutDeleteTimer( g_startTime));
	return result;
}

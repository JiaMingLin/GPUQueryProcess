...
void preallocBlockSums(unsigned int maxNumElements)
{
    ...
    {       
        ...
        if (numBlocks > 1) 
        {
            GPUMALLOC((void**) &g_scanBlockSums[level++],  
                           ...
                           int level)
{
    ...
    if (numBlocks > 1)
    {
        ...
        prescan<true, false><<< grid, threads, sharedMemSize >>>(outArray, 
                                                                 ...
                                                                 numThreads * 2, 0, 0);
#else
		GPUMALLOC( (void**)&d_data, sharedMemSize*grid.x );
		prescan<true, false><<< grid, threads>>>(d_data, outArray, 
                                                                 inArray, 
                                                                 g_scanBlockSums[level],
                                                                 numThreads * 2, 0, 0, sharedMemSize);
		GPUFREE( d_data );
#endif
...
#else
			GPUMALLOC( (void**)&d_data, sharedMemLastBlock*1 );
            prescan<true, true><<< 1, numThreadsLastBlock>>>
                (d_data, outArray, inArray, g_scanBlockSums[level], numEltsLastBlock, 
                 numBlocks - 1, numElements - numEltsLastBlock, sharedMemLastBlock );
			GPUFREE( d_data );
#endif
...
#else
		GPUMALLOC( (void**)&d_data, sharedMemSize*grid.x );
        prescan<false, false><<< grid, threads >>>(d_data, outArray, inArray,
                                                                  0, numThreads * 2, 0, 0, sharedMemSize);
		GPUFREE( d_data );
#endif
...
#else
		GPUMALLOC( (void**)&d_data, sharedMemSize*grid.x );
         prescan<false, true><<< grid, threads>>>(d_data, outArray, inArray, 
                                                                  0, numElements, 0, 0, sharedMemSize );
		 GPUFREE( d_data );
#endif
...
void prescanArray(int *outArray, int *inArray, int numElements)
{
	...
	#else
		printf( "NO COALESCED, SCAN\n" );
	#endif

	#ifdef SHARED_MEM
		printf( "YES, SHARED MEMORY, SCAN\n" );
	#else
		printf( "NO SHARED MEMORY, SCAN\n" );
	#endif
#endif

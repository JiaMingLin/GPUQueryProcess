...
void scatter_thread(Record *R, int *loc, Record *S, int rLen, int numThread);
void gather_thread(Record *R, int *loc, Record *S, int rLen, int numThread);
...
void sort_openmp(Record* Rin, int rLen, cmp_func fcn, mapper_t mapFunc, Record* Rout, int numThread);
...
template <class T> void mapImpl(Record *Rin, int rLen, mapper_t mapFunc, void* para, T* Rout, int numThread)
{
#ifndef SAVEN_OPen_MP
	mapImpl_thread<T>(Rin,rLen,mapFunc, para,Rout, numThread);
#else 
	mapImpl_openmp<T>(Rin,rLen,mapFunc,para, Rout);
#endif
}

int scan(Record *Rin, int rLen, int numThread, int* pS);
void scatter(Record *R, int *loc, Record *S, int rLen, int numThread);
void gather(Record *R, int *loc, Record *S, int rLen, int numThread);
